...
def run_simulation(
    ...
    geo = dismech.Geometry.from_txt(input_txt, plot_from_txt=False)

    robot = dismech.SoftRobot(geom, material, geo, dyn_sim, env)

    # --- grip nodes ---
    main_grip = [27]  # as list to match original usage
    mg = main_grip[0]  # int for arithmetic

    other_grip_nodes_below = np.arange(mg - 4, mg, dtype=int)
    # [23, 24, 25, 26]

    other_grip_nodes_above = np.arange(mg + 4, mg, -1, dtype=int)
    # [31, 30, 29, 28]
