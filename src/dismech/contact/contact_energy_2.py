...
class ContactEnergy(metaclass=abc.ABCMeta):
    ...
    def get_energy(self, q, output_scalar: bool = True):
        ...
        E, _, _ = compute_energy_grad_hess(
            Delta, self.norm_delta, self.norm_h, self.norm_k_1)
    ...
    def grad_hess_energy(self, state, robot, F, first_iter):
        ...
        E, grad_E_D, hess_E_D = compute_energy_grad_hess(
            Delta, self.norm_delta, self.norm_h, self.norm_k_1)
    ...
    def get_candidate_pairs(self, state: RobotState):
        min_dist = self.norm_h + 100 * self.norm_delta
        delta = self.get_Delta(state.q) / self.scale
        return delta < min_dist
    ...
    def get_contact_stiffness(self, robot, F: np.ndarray):
        ...
        valid_dofs = robot.map_node_to_dof(
            self.pairs)
        force_per_stencil = np.max(np.linalg.norm(F[valid_dofs], axis=2), axis=1)
