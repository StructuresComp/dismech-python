...
class StretchEnergy(ElasticEnergy):
    def __init__(self, springs: typing.List[StretchSpring]):
        ...
        self.inv_l_k = 1.0 / self.l_k  # Precompute inverse reference lengths

        super().__init__(
            self.l_k * np.array([s.EA for s in springs]),
            np.zeros_like(self.l_k),
            np.array([s.nodes_ind for s in springs]),
            np.array([s.ind for s in springs])
        )

        n_springs = self.l_k.shape[0]
        # Pre-allocate all arrays
        self.grad_eps = np.empty((n_springs, 6), dtype=np.float64)
        self.hess_eps = np.empty((n_springs, 6, 6), dtype=np.float64)
        self.edge_outer = np.empty((n_springs, 3, 3), dtype=np.float64)
        self.M = np.empty((n_springs, 3, 3), dtype=np.float64)
        self.M2 = np.empty((n_springs, 3, 3), dtype=np.float64)
    ...
    def get_strain(self, q: np.ndarray,
        ...
        return edge_len * self.inv_l_k - 1.0  # edge_len/l_k - 1
