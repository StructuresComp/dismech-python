...
class HingeSpring:
    ...
    def get_grad_theta(x0, x1, x2, x3):
        ...
        m_cosA1 = np.sum(m_e0 * m_e1, axis=1, keepdims=True) / (norm_e0 * norm_e1)
        m_cosA2 = np.sum(m_e0 * m_e2, axis=1, keepdims=True) / (norm_e0 * norm_e2)
        m_cosA3 = -np.sum(m_e0 * m_e3, axis=1, keepdims=True) / (norm_e0 * norm_e3)
        m_cosA4 = -np.sum(m_e0 * m_e4, axis=1, keepdims=True) / (norm_e0 * norm_e4)

        # Compute sine terms using cross products
        cross_e0_e1 = np.cross(m_e0, m_e1)
        m_sinA1 = np.linalg.norm(cross_e0_e1, axis=1, keepdims=True) / (norm_e0 * norm_e1)
        
        cross_e0_e2 = np.cross(m_e0, m_e2)
        m_sinA2 = np.linalg.norm(cross_e0_e2, axis=1, keepdims=True) / (norm_e0 * norm_e2)
        
        cross_e0_e3 = np.cross(m_e0, m_e3)
        m_sinA3 = -np.linalg.norm(cross_e0_e3, axis=1, keepdims=True) / (norm_e0 * norm_e3)
        
        cross_e0_e4 = np.cross(m_e0, m_e4)
        m_sinA4 = -np.linalg.norm(cross_e0_e4, axis=1, keepdims=True) / (norm_e0 * norm_e4)
    ...
    def get_grad_hess_theta(x0, x1, x2, x3):
        ...
        M331 = (m_cosA3 / (m_h3**2))[..., None] * np.einsum('ni,nj->nij', m_m3, m_nn1)
        M311 = (m_cosA3 / (m_h3 * m_h1))[..., None] * np.einsum('ni,nj->nij', m_m1, m_nn1)
        M131 = (m_cosA1 / (m_h1 * m_h3))[..., None] * np.einsum('ni,nj->nij', m_m3, m_nn1)
        M3011 = (m_cosA3 / (m_h3 * m_h01))[..., None] * np.einsum('ni,nj->nij', m_m01, m_nn1)
        M111 = (m_cosA1 / (m_h1**2))[..., None] * np.einsum('ni,nj->nij', m_m1, m_nn1)
        M1011 = (m_cosA1 / (m_h1 * m_h01))[..., None] * np.einsum('ni,nj->nij', m_m01, m_nn1)

        M442 = (m_cosA4 / (m_h4**2))[..., None] * np.einsum('ni,nj->nij', m_m4, m_nn2)
        M422 = (m_cosA4 / (m_h4 * m_h2))[..., None] * np.einsum('ni,nj->nij', m_m2, m_nn2)
        M242 = (m_cosA2 / (m_h2 * m_h4))[..., None] * np.einsum('ni,nj->nij', m_m4, m_nn2)
        M4022 = (m_cosA4 / (m_h4 * m_h02))[..., None] * np.einsum('ni,nj->nij', m_m02, m_nn2)
        M222 = (m_cosA2 / (m_h2**2))[..., None] * np.einsum('ni,nj->nij', m_m2, m_nn2)
        M2022 = (m_cosA2 / (m_h2 * m_h02))[..., None] * np.einsum('ni,nj->nij', m_m02, m_nn2)

        B1 = (1 / (norm_e0**2))[..., None] * np.einsum('ni,nj->nij', m_nn1, m_m01)
        B2 = (1 / (norm_e0**2))[..., None] * np.einsum('ni,nj->nij', m_nn2, m_m02)

        N13 = (1 / (m_h01 * m_h3))[..., None] * np.einsum('ni,nj->nij', m_nn1, m_m3)
        N24 = (1 / (m_h02 * m_h4))[..., None] * np.einsum('ni,nj->nij', m_nn2, m_m4)
        N11 = (1 / (m_h01 * m_h1))[..., None] * np.einsum('ni,nj->nij', m_nn1, m_m1)
        N22 = (1 / (m_h02 * m_h2))[..., None] * np.einsum('ni,nj->nij', m_nn2, m_m2)
        N101 = (1 / (m_h01**2))[..., None] * np.einsum('ni,nj->nij', m_nn1, m_m01)
        N202 = (1 / (m_h02**2))[..., None] * np.einsum('ni,nj->nij', m_nn2, m_m02)
        ...
        hessTheta[:, 0:3, 3:6] = M311 + np.swapaxes(M131, -1, -2) + B1 + M422 + np.swapaxes(M242, -1, -2) + B2
